...
void i2cencoder_setup()
{
  ...
  for (int enc_cnt = 0; enc_cnt < NUMBER_OF_ENCODERS; enc_cnt++) {
      ...
      Encoder[enc_cnt].writeMax((int32_t)127); //SEt the maximum threshold to 5
      Encoder[enc_cnt].writeMin((int32_t) 0); //Set the minimum threshold to 0
      Encoder[enc_cnt].writeStep((int32_t)1); //The step at every encoder click is 0.5
