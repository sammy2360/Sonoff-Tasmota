...
enum TasmotaCommands {
  ...
  CMND_COUNTERDEBOUNCE, CMND_BUTTONDEBOUNCE, CMND_ENCODER, CMND_ENCODERMAX, CMND_ENCODERMIN, CMND_ENCODERSTEPS, CMND_SWITCHDEBOUNCE, CMND_SLEEP, CMND_UPGRADE, CMND_UPLOAD, CMND_OTAURL, CMND_SERIALLOG, CMND_SYSLOG,
...
const char kTasmotaCommands[] PROGMEM =
  ...
  D_CMND_COUNTERDEBOUNCE "|" D_CMND_BUTTONDEBOUNCE "|" D_CMND_ENCODER "|" D_CMND_ENCODERMAX "|" D_CMND_ENCODERMIN "|" D_CMND_ENCODERSTEPS "|" D_CMND_SWITCHDEBOUNCE "|" D_CMND_SLEEP "|" D_CMND_UPGRADE "|" D_CMND_UPLOAD "|" D_CMND_OTAURL "|" D_CMND_SERIALLOG "|" D_CMND_SYSLOG "|"
...
//      Settings.flag.device_index_enable = user_append_index;
    ...
    else if (CMND_ENCODERMAX == command_code) {
      if ((payload > 0) && (payload < 128)) {
        Settings.encoder_max_limit = payload;
        i2cencoder_setup();
      }
      snprintf_P(mqtt_data, sizeof(mqtt_data), S_JSON_COMMAND_NVALUE, command, Settings.encoder_max_limit);
    }
    else if (CMND_ENCODERMIN == command_code) {
      if ((payload > 0) && (payload < 128)) {
        Settings.encoder_min_limit = payload;
        i2cencoder_setup();
      }
      snprintf_P(mqtt_data, sizeof(mqtt_data), S_JSON_COMMAND_NVALUE, command, Settings.encoder_min_limit);
    }
    else if (CMND_ENCODERSTEPS == command_code) {
      if ((payload > 0) && (payload < 21)) {
        Settings.encoder_steps = payload;
        i2cencoder_setup();
      }
      snprintf_P(mqtt_data, sizeof(mqtt_data), S_JSON_COMMAND_NVALUE, command, Settings.encoder_steps);
    }
